...
int main(void){
	...
	int  decide2ATM3,ATMtodecide; 
	int msgLength = sizeof(my_msg_st) -sizeof(long);


	decide2ATM3 =msgget(ATM3_KEY, IPC_CREAT | 0600);
	if (decide2ATM3 == -1) { 
		perror("Creating ATM message queue."); msgctl(decide2ATM3, IPC_RMID,0); exit(1);
	}

	ATMtodecide =msgget(allAtms_KEY, IPC_CREAT | 0600);
	if (ATMtodecide == -1) { 
		perror("Creating DB message queue."); exit(1); 
	}
	...
	while(shared_stuff->close != 'x'){
		while(true){
			...
			while(!goodPin){
				...
				{ 
					fprintf(stderr, "\n pin3 msgsnd failed ATM\n");	
					exit(EXIT_FAILURE);	
					}

				if (msgrcv(decide2ATM3, (void *)&recieve, msgLength, msg_to_receive, 0) == -1) { 
		...
		while(true){
			...
			if (!strcmp(temp, "C")){
				...
				printf("type in desposit amount:");
				scanf("%f",&send.info.funds);
				fflush(stdin);
				printf("\n");
				break;
		...
		if (recieve.type == notEnoughFunds)
		{
			...
			printf("\nyour account balance is now: %f after withdrawal\n", recieve.info.funds);
		}
	...
	if (msgctl(decide2ATM3, IPC_RMID, 0) == -1){fprintf(stderr, " failed to clsoe decide2ATM1 msg que\n"); exit(EXIT_FAILURE); }

    if (msgctl(ATMtodecide, IPC_RMID, 0) == -1){fprintf(stderr, "failed to close ATMtodecide msg que\n"); exit(EXIT_FAILURE); }
