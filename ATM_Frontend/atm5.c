...
int main(void){
	...
	int decide2ATM5=msgget(ATM5_KEY, IPC_CREAT | 0600);
	if (decide2ATM5 == -1) {
	...
	while(shared_stuff->close != 'x'){
		...
		while(true){
			...
			while(!goodPin){
				...
				if (msgrcv(decide2ATM5, (void *)&recieve, msgLength, msg_to_receive, 0) == -1)
				{
	...
	if (msgctl(decide2ATM5, IPC_RMID, 0) == -1)
    {
