	...
	while(shared_stuff->close != 'x'){
		while(true){
			...
			while(true){
				while( true){
					FILE *fpr;
					fpr = fopen("logLL.txt", "a+");
					semaphore_p(sem_id1);
					printf("acquired semaphore 1\n");
					printf("\nPlease enter account number: \n");
					scanf("%s",send.info.account);	
					fflush(stdin);
					fprintf(fpr, "atm1 acquired semaphore account %s\n" ,&send.info.account);
					//checks to close atm
					if( send.info.account[0] == 'X' || send.info.account[0] == 'x' ){
						...
						printf("\nATM: Inputed Pin  %s\n",send.info.pin);
						fprintf(fpr, "atm1 got pin %s\n", &send.info.pin);
					
					}
					fprintf(fpr, "atm1 about to get semaphore 2\n");
				
					semaphore_v(sem_id1);
					fprintf(fpr, "atm1 released semaphore 1\n");
					sleep(10);
					semaphore_p(sem_id2);
					fprintf(fpr, "atm1 acquired semaphore 2\n");
					sleep(10);
					semaphore_v(sem_id2);
					fprintf(fpr, "atm1 released semaphore 2\n");
